...
import { ActionReceipt } from '../src/agent/actionExecutor.js';
...
describe('RAVEN M11 — Goal State, Action Memory & Repetition Prevention Test Suite', () => {
  ...
  it('9. ActionGuard rejects execution when goal is already complete', () => {
    ...
    const memory = new ActionMemory();
    const res = ActionGuard.shouldExecuteAction(
      { action: 'CLICK', targetSelector: 'login-btn', value: null },
      { goalManager: gm, actionMemory: memory, currentScreenElements: [{ id: 'login-btn' }], currentPageFingerprint: 'fp_1' }
    );
  ...
  it('10. ActionGuard rejects action that targets an already completed sub-goal', () => {
    ...
    const memory = new ActionMemory();
    const res = ActionGuard.shouldExecuteAction(
      { action: 'CLICK', targetSelector: 'search-field-id', value: null, reasoning: 'Find search field' },
      { goalManager: gm, actionMemory: memory, currentScreenElements: [{ id: 'search-field-id' }], currentPageFingerprint: 'fp_1', proposedTargetText: 'Find search field' }
    );
  ...
  it('11. ActionGuard rejects action when target is missing/stale', () => {
    ...
    const res = ActionGuard.shouldExecuteAction(
      { action: 'CLICK', targetSelector: 'missing-btn-777', value: null },
      { goalManager: gm, actionMemory: memory, currentScreenElements: [{ id: 'other-btn' }], currentPageFingerprint: 'fp_1' }
    );
  ...
  it('22. Same element on next page re-observation does NOT cause repeated action', async () => {
    ...
    const guardRes = ActionGuard.shouldExecuteAction(
      { action: 'CLICK', targetSelector: 'login-btn', value: null },
      { goalManager: controller.goalManager, actionMemory: controller.actionMemory, currentScreenElements: [{ id: 'login-btn' }], currentPageFingerprint: 'fp_same' }
    );
  ...
  it('23. Failed action retries up to maxActionRetries limit (2 retries)', async () => {
    ...
    let attempts = 0;
    const dispatchFail = async (): Promise<ActionReceipt> => {
      attempts++;
