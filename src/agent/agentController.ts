...
import { createPageFingerprint, PageFingerprintResult } from './pageFingerprint.js';
...
export function assertValidPhaseTransition(previous: AgentPhase, next: AgentPhase): boolean {
...
export function actionFingerprint(command: ValidatedCommand): string {
  return [
    command.action,
    command.targetSelector ?? '',
    command.value ?? '',
    (command as any).direction ?? ''
  ].join('|');
}

export function computeObservationHash(url: string, elements: any[]): string {
...
export function serverRequestFingerprint(taskId: string, subGoal: string, observationHash: string): string {
...
export class AgentController {
  ...
  public currentTaskIntent?: TaskIntent;
  public currentIteration: number = 1;
  public maxIterations: number = 10;
  public maxActionRetries: number = 2;
  public currentActionRetries: number = 0;
  public status: AgentStatus = 'IDLE';
  public executionHistory: AgentExecutionRecord[] = [];

  public goalManager: GoalManager = new GoalManager();
  public actionMemory: ActionMemory = new ActionMemory();

  public privacyChecksCount: number = 0;
  public protectedItemsCount: number = 0;
  public serverDecisionsCount: number = 0;

  public taskState: AgentTaskState;

  private stabilizeDelayMs: number = 600;
  private previousFingerprint?: PageFingerprintResult;

  constructor(config?: { maxIterations?: number; maxActionRetries?: number; stabilizeDelayMs?: number }) {
    if (config?.maxIterations !== undefined) this.maxIterations = config.maxIterations;
    if (config?.maxActionRetries !== undefined) this.maxActionRetries = config.maxActionRetries;
    if (config?.stabilizeDelayMs !== undefined) this.stabilizeDelayMs = config.stabilizeDelayMs;
    ...
    this.taskState = {
      ...
      completedSubGoals: [],
      actionHistory: [],
      executedActionFingerprints: new Set<string>(),
      serverRequestFingerprints: new Set<string>(),
  ...
  public getTaskState(): AgentTaskState {
    return {
      ...this.taskState,
  ...
  public transitionToPhase(nextPhase: AgentPhase): boolean {
  ...
  public completeTask(reason: string): void {
   ...
   */
  public async observeCurrentPage(
    queryDomFn: () => Promise<ElementInfo[]>,
    runPerceptionFn: () => Promise<any>
  ): Promise<{ sanitizedPayload: any; currentFingerprint: PageFingerprintResult; stepRedactedCount: number }> {
  ...
  public async executeIteration(
    ...
    let currentFingerprint: PageFingerprintResult;
    let stepRedactedCount: number;
  ...
  private async executeAndVerifyAction(
    ...
    if (verifiedSuccess) {
      ...
      this.recordStep({
        step: currentStep,
        goal: this.taskGoal,
        status: (this.taskState.taskCompleted || (this.status as string) === 'COMPLETED') ? 'COMPLETED' : 'PHASE_3_VERIFICATION',
        privacySafe: true,
        redactedCount: 0,
      ...
      if (this.taskState.taskCompleted || this.taskState.stopped || (this.status as string) === 'COMPLETED') {
   ...
   */
  private getFastPathCommand(elements: any[]): ValidatedCommand | null {
    ...
    if ((intent?.intent === 'SEARCH' || intent?.intent === 'TYPE') && intent.value?.value) {
      ...
      const targetEl = elements.find((el: any) => {
