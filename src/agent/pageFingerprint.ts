...
export function createPageFingerprint(sanitizedPageState: any): PageFingerprintResult {
...
function isPotentialPii(text: string): boolean {
  ...
  if (lower.includes('@') && lower.includes('.')) return true; // Email
  if (/(?:\+?\d{1,3}[\s\-.]?)?\(?\d{2,4}\)?[\s\-.]?\d{3,4}[\s\-.]?\d{3,4}/.test(text)) return true; // Phone
  if (/\b\d{4}[\s\-]?\d{4}[\s\-]?\d{4}[\s\-]?\d{4}\b|\b\d{13,19}\b/.test(text)) return true; // Card
  if (/\b\d{12}\b|\b\d{4}\s\d{4}\s\d{4}\b/.test(text)) return true; // Aadhaar
  return false;
}
