...
export class TaskIntentParser {
   ...
   */
  public static parseGoal(rawGoal: string): TaskIntent {
    ...
    if (lower.includes('search')) {
      ...
      const actionValue: ActionValue | undefined = extractedValue ? {
    ...
    if (lower.includes('scroll')) {
      const dir: 'UP' | 'DOWN' = lower.includes('up') ? 'UP' : 'DOWN';
    ...
    const defaultParsed: TaskIntent = {
      rawGoal: goalStr,
      intent: 'MULTI_STEP',
      subGoals: [
   ...
   */
  private static extractExplicitValue(goal: string, context: string): string | undefined {
  ...
  private static extractTarget(goal: string, defaultTarget: string): string {
  ...
  private static logDiagnostics(intent: TaskIntent): void {
