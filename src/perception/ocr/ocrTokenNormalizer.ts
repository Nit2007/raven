...
export class OcrTokenNormalizer {
  private static readonly MAX_BASELINE_DIFF_PX = 12;
  private static readonly MAX_HORIZONTAL_GAP_PX = 45;

  /**
   * Scans raw word-level OCR tokens and groups spatially adjacent tokens into normalized line-level text regions.
   */
  public normalizeTokens(rawTokens: RawOcrWord[]): NormalizedOcrRegion[] {
    if (!rawTokens || rawTokens.length === 0) return [];

    const normalizedRegions: NormalizedOcrRegion[] = [];
    ...
    for (let i = 0; i < rawTokens.length; i++) {
      ...
      const groupTokens: RawOcrWord[] = [currentToken];
      usedIndices.add(i);
      ...
      for (let j = i + 1; j < rawTokens.length; j++) {
        ...
        if (dy > OcrTokenNormalizer.MAX_BASELINE_DIFF_PX) continue;
   ...
   */
  private computeCombinedBBox(tokens: RawOcrWord[]): BoundingBox {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
