...
export interface PerceptionFusionInput {
  ...
  faceResults?: { detections: DetectionResult[]; status: 'SUCCESS' | 'FAILED' | 'SKIPPED'; error?: string };
  ocrResults?: { detections: DetectionResult[]; status: 'SUCCESS' | 'FAILED' | 'SKIPPED'; error?: string };
  piiResults?: { detections: DetectionResult[]; status: 'SUCCESS' | 'FAILED' | 'SKIPPED'; error?: string };
  visionResults?: { detections: DetectionResult[]; status: 'SUCCESS' | 'FAILED' | 'SKIPPED'; error?: string };
  timing: StageTiming;
}
...
export class PerceptionFusionEngine {
   ...
   */
  public static computeIoU(a: BoundingBox, b: BoundingBox): number {
   ...
   */
  public static validateAndClampBBox(bbox: BoundingBox, imgWidth: number, imgHeight: number): BoundingBox {
   ...
   */
  public fuseDetections(
    detectionGroups: DetectionResult[][],
    imgWidthOrIou: number = 1920,
    imgHeight = 1080,
    iouThreshold = 0.5
  ): DetectionResult[] {
    ...
    const rawAll: DetectionResult[] = detectionGroups.flat();
    if (rawAll.length === 0) return [];

    // 1. Validate & clamp coordinates for all input detections
    const allDetections: DetectionResult[] = rawAll.map(det => ({
      ...det,
    ...
    for (let i = 0; i < allDetections.length; i++) {
      ...
      for (let j = i + 1; j < allDetections.length; j++) {
        ...
        if (current.type === candidate.type && iou >= iouThresh) {
          const currentText = (current.metadata?.text as string || '').trim();
          const candidateText = (candidate.metadata?.text as string || '').trim();
   ...
   */
  public buildUnifiedResult(input: PerceptionFusionInput): UnifiedPerceptionResult {
    ...
    const faceSubsys: SubsystemStatus = faceResults ? { status: faceResults.status, error: faceResults.error } : { status: 'SKIPPED' };
    const ocrSubsys: SubsystemStatus = ocrResults ? { status: ocrResults.status, error: ocrResults.error } : { status: 'SKIPPED' };
    const piiSubsys: SubsystemStatus = piiResults ? { status: piiResults.status, error: piiResults.error } : { status: 'SKIPPED' };
    const visionSubsys: SubsystemStatus = visionResults ? { status: visionResults.status, error: visionResults.error } : { status: 'SKIPPED' };
