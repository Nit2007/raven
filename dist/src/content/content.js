...
(() => {
  ...
  (function() {
    if (window.__RAVEN_CONTENT_INITIALIZED__) {
      console.log("[RAVEN Content Script] Already initialized in tab.");
      return;
    }
    window.__RAVEN_CONTENT_INITIALIZED__ = true;
    ...
    function extractLiveDomElements() {
      ...
      rawNodes.forEach((node, idx) => {
        ...
        const textVal = (node.textContent || node.value || node.placeholder || "").trim();
        ...
        results.push({
          ...
          name: node.getAttribute("name") || void 0,
          id: node.id || `el_${idx}`,
    ...
    function extractRawDomNodeList() {
    ...
    function findTargetElement(targetSelector) {
      ...
      try {
        const elByCss = document.querySelector(targetSelector);
        if (elByCss) return elByCss;
      } catch (_) {
      }
      const elByName = document.querySelector(`[name="${targetSelector}"], [value="${targetSelector}"]`);
      if (elByName) return elByName;
      const candidates = extractRawDomNodeList();
      for (const cand of candidates) {
        const text = (cand.textContent || cand.value || "").trim().toLowerCase();
        if (text && text === lowerTarget) return cand;
        if (text && text.includes(lowerTarget)) return cand;
      }
      return null;
    }
    async function executeValidatedActionAsync(command) {
      ...
      if (actionType === "CLICK") {
        ...
        const label = targetEl.textContent?.trim() || targetEl.value || targetSelector || "element";
        return {
          success: true,
          action: "CLICK",
          target_element_id: targetSelector,
