    ...
    detectPiiFromOcr(ocrResults) {
        ...
        for (let i = 0; i < ocrResults.length; i++) {
            ...
            if (PiiCandidateDetector.EMAIL_REGEX.test(text)) {
                ...
                if (hasLabel)
                    evidence.push('EMAIL_LABEL_CONTEXT');
                const confidence = Math.min(0.99, Math.max(0.85, item.confidence * (hasLabel ? 1.05 : 0.98)));
                detections.push(this.createDetection(idCounter++, 'EMAIL', text, item.bbox, confidence, evidence));
                continue;
            }
            ...
            if (text.startsWith('+')) {
                ...
                if (phoneSeq) {
                    ...
                    if (!hasNonPhoneLabel || hasPhoneLabel) {
                        ...
                        if (phoneSeq.tokenCount > 1)
                            evidence.push('MULTI_TOKEN_SPATIAL_GROUPING');
                        if (hasPhoneLabel)
                            evidence.push('PHONE_LABEL_CONTEXT');
                        let confidence = phoneSeq.avgConfidence * (hasPhoneLabel ? 1.05 : 0.88);
                        if (hasNonPhoneLabel && !hasPhoneLabel)
                            confidence *= 0.40;
                        if (confidence >= 0.70 && (!hasNonPhoneLabel || hasPhoneLabel)) {
                            detections.push(this.createDetection(idCounter++, 'PHONE', phoneSeq.combinedText, phoneSeq.mergedBBox, Math.min(0.99, confidence), evidence));
            ...
            if (cardSeq) {
                ...
                if (isLuhnValid || hasCardLabel) {
                    ...
                    if (cardSeq.tokenCount > 1)
                        evidence.push('MULTI_TOKEN_SPATIAL_GROUPING');
                    if (isLuhnValid)
                        evidence.push('LUHN_CHECKSUM_VALID');
                    if (hasCardLabel)
                        evidence.push('CARD_LABEL_CONTEXT');
                    const confidence = Math.min(0.99, Math.max(0.85, cardSeq.avgConfidence * (isLuhnValid ? 1.05 : 0.90)));
                    detections.push(this.createDetection(idCounter++, 'PAYMENT_CARD', cardSeq.combinedText, cardSeq.mergedBBox, confidence, evidence));
            ...
            if (PiiCandidateDetector.PAN_REGEX.test(text)) {
                detections.push(this.createDetection(idCounter++, 'GOVERNMENT_ID', text, item.bbox, Math.min(0.99, item.confidence * 0.98), ['PAN_CARD_PATTERN']));
                continue;
            }
            if (PiiCandidateDetector.SSN_REGEX.test(text)) {
                detections.push(this.createDetection(idCounter++, 'GOVERNMENT_ID', text, item.bbox, Math.min(0.99, item.confidence * 0.98), ['SSN_PATTERN']));
                continue;
            }
            ...
            if (aadhaarSeq) {
                ...
                if (hasAadhaarLabel) {
                    detections.push(this.createDetection(idCounter++, 'GOVERNMENT_ID', aadhaarSeq.combinedText, aadhaarSeq.mergedBBox, Math.min(0.99, aadhaarSeq.avgConfidence * 1.02), ['AADHAAR_PATTERN', 'GOVT_ID_LABEL_CONTEXT']));
            ...
            if (hasNameLabel && /^[A-Z][a-z]+$/.test(text) && text !== 'Name' && text !== 'User') {
                ...
                detections.push(this.createDetection(idCounter++, 'PERSON_NAME', combinedText, combinedBBox, Math.min(0.98, Math.max(0.85, item.confidence * 0.95)), ['NAME_LABEL_CONTEXT', 'CAPITALIZED_NAME_PATTERN']));
                i += (consumedCount - 1);
                continue;
            }
            ...
            if (hasAddressLabel && text.length >= 3 && !/^(address|street|city|zip)$/i.test(text)) {
                detections.push(this.createDetection(idCounter++, 'ADDRESS', text, item.bbox, Math.min(0.95, item.confidence * 0.88), ['ADDRESS_LABEL_CONTEXT']));
                continue;
            }
            ...
            if (hasPasswordLabel || isSecretKey) {
                const evidence = [];
                if (isSecretKey)
                    evidence.push('API_SECRET_PATTERN');
                if (hasPasswordLabel)
                    evidence.push('PASSWORD_LABEL_CONTEXT');
                detections.push(this.createDetection(idCounter++, 'PASSWORD', text, item.bbox, Math.min(0.99, isSecretKey ? 0.99 : item.confidence * 0.95), evidence));
                continue;
            }
        }
