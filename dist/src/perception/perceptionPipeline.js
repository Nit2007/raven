...
export class LocalPerceptionPipeline {
    ...
    async runLocalPerception(input, imageSource) {
        ...
        const faceRes = faceResp.success
            ? { detections: faceResp.detections, status: 'SUCCESS' }
            : { detections: [], status: 'FAILED', error: faceResp.error };
        const rawOcrWords = ocrResp.words || [];
        const ocrRes = ocrResp.success
            ? { detections: ocrResp.detections, status: 'SUCCESS' }
            : { detections: [], status: 'FAILED', error: ocrResp.error };
