...
async function runLoop(tabId) {
  ...
  try {
    ...
    while (!state.stopped && state.iteration < MAX_ITERATIONS && state.status === 'running') {
      ...
      // faces on the M1 screenshot before any data leaves the page)
      try {
        await runM5PiiAnalysis(tabId, { iteration: state.iteration });
      } catch (m5Err) {
        console.warn('[M5 PII] Non-fatal face/PII detection error during loop:', m5Err);
      }
